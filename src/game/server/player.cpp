...
CPlayer::CPlayer(CGameContext *pGameServer, int ClientID, int Team)
{
    ...
    m_EyeEmoteDuration = -1;
    m_DefaultEyeEmote = EMOTE_NORMAL;
    ...
    m_MuteTick = 0;
    m_ToxicStrikes = 0;
    m_ResetStrikesAfterMute = false;
    
    m_MultiCount = 0;
    m_CurrentSpree = 0;
    m_LastKillTick = -1;
    m_MultiEndAnnounceTick = -1;
    m_LastCommandRequestTick = -Server()->TickSpeed() * 10;
    str_copy(m_aOriginalName, Server()->ClientName(ClientID), sizeof(m_aOriginalName));
}
...
void CPlayer::Tick()
{
    ...
    if(m_aSavedName[0] == '\0' && Server()->ClientName(m_ClientID)[0])
    {
        str_copy(m_aSavedName, Server()->ClientName(m_ClientID), sizeof(m_aSavedName));
        //dbg_msg("stats", "Saved name set in Tick: '%s'", m_aSavedName);

        // Load stats after setting name
        GameServer()->LoadPlayerStatsFromFile(this);
        GameServer()->LoadRoundStatsFromFile(this);
    }
	...
	// velocity stuff
	m_ViewVel = (m_ViewPos - m_LastViewPos) * Server()->TickSpeed();
	...
	if(!GameServer()->m_World.m_Paused)
	{
		...
		if(m_EmotionDuration != 0)
			--m_EmotionDuration;
        if(m_EyeEmoteDuration > 0)
            m_EyeEmoteDuration--;
        else if(m_EyeEmoteDuration == 0)
        {
            m_EyeEmoteDuration = -1;
            GameServer()->SetEyeEmoteDefault(m_ClientID);
        }
	}
	else
	{
...
void CPlayer::KillCharacter(int Weapon, bool forced)
{
    ...
    {
        Respawn();
        m_RespawnTick = Server()->Tick();  // Just in case respawn logic checks this
    }
}
...
void CPlayer::ResetRoundStats()
{
	...
	m_RoundSaves = 0;
	m_RoundSavedBy = 0;
	m_RoundGreenSpikeKills = 0;
	m_RoundGoldSpikeKills = 0;
	m_RoundPurpleSpikeKills = 0;
	m_RoundWrongShrineKills = 0;
	m_RoundSteals = 0;
	m_RoundStolenFrom = 0;
	m_RoundMaxSpree = 0;
	m_RoundMaxMulti = 0;
...
void CPlayer::Init(int ClientID)
{
    // if we already had a character, destroy it
    if(m_pCharacter)
    {
        delete m_pCharacter;
    ...
    // zero out all FNG-specific stats
    ResetStats();
}
